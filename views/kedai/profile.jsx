...
module.exports = function ({user, kedai, editMode}) {
    return <Layout title={editMode ? "Edit Kedai" : kedai && kedai.name}>
        <h1>{user && user.username}</h1>
        <form action="" className={"flex flex-col"}>
            <input type="text" name="name" className={"input-type"} id="name" disabled={!editMode} placeholder={"Kedai name"} defaultValue={kedai && kedai.name}/>
            <textarea name="description" className={"input-type"} id="description" cols="30" rows="10" disabled={!editMode}
